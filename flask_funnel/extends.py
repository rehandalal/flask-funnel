...
def coffee(sin, sout, **kw):
    ...
    subprocess.call([current_app.config.get("COFFEE_BIN", "coffee"), "-s", "-c"], stdin=sin, stdout=sout)

@extend(accept=".less", export=".css")
def less(sin, sout, **kw):
    subprocess.call([current_app.config.get("LESS_BIN", "lessc"), "-"], stdin=sin, stdout=sout)

@extend(accept=".scss", export=".css")
def scss(sin, sout, **kw):
    subprocess.call([current_app.config.get("SCSS_BIN", "scss"), "-s"], stdin=sin, stdout=sout)
...
def produce(filepath, relate_filepath=None):
    ...
    if relate_filepath is None:
        ...
        relate_filepath = os.path.join(current_app.config.get('BUNDLES_DIR'), "tmp", name)
    else:
        # remove the postfix
    ...
    for accept, export, func in mapping:
        ...
        if accept == postfix:
            ...
            target_path = os.path.join(current_app.static_folder, relate_filepath)
